...
=======
    ...
    private fun checkGraphForCycle(graph: Graph, current: Vertex, visited: ArrayList<Vertex>, checkList: ArrayList<Boolean>) {
        visited.add(current)
...
=======
    ...
    fun TopSortActions(graph: Graph): Pair<Map<Vertex, Int>, ArrayList<State>> {
        ...
        if (false in checkList) return Pair(mapOf(), arrayListOf(State(getCurrentTime(), "Cycle was found, create another graph", 0)))
        visited = arrayListOf()
        for (vertex in vertexes) {
            if (vertex !in visited) {
